...
export default function CashBookPage() {
    ...
    useEffect(() => {
        document.title = "Cash Book | Finance | ProductionFlow ERP";
    ...
    // Filter for cash transactions (account ID 1001 = Cash)
    const cashTransactions = journals.filter(j => {
        const hasDate = !fromDate || !toDate || (j.date >= fromDate && j.date <= toDate);
        const hasCash = j.entries.some(e => e.accountId === "1001");
        return hasDate && hasCash;
    }).flatMap(j =>
        j.entries
            .filter(e => e.accountId === "1001")
            .map(e => ({
                date: j.date,
    ...
    const transactionsWithBalance = cashTransactions.map(t => {
        runningBalance += (t.debit - t.credit);
        return { ...t, balance: runningBalance };
    });

    const totalIn = cashTransactions.reduce((sum, t) => sum + t.debit, 0);
    const totalOut = cashTransactions.reduce((sum, t) => sum + t.credit, 0);
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-6">
            {/* Header */}
            <div>
                <h1 className="text-3xl font-bold text-slate-900">Cash Book</h1>
                <p className="text-slate-500 mt-1">View all cash transactions and balances</p>
            </div>
                ...
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-sm text-green-600">Cash In</div>
                        </div>
                    </div>
                </div>
                <div className="bg-red-50 rounded-lg p-4 border border-red-200">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            ...
                            <div className="text-sm text-red-600">Cash Out</div>
                        </div>
                    </div>
                </div>
                ...
                <table className="w-full text-left text-sm">
                    <thead className="bg-slate-50 border-b border-slate-100">
                        <tr>
                            ...
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">Cash In</th>
                            <th className="px-6 py-4 font-semibold text-slate-700 text-right">Cash Out</th>
