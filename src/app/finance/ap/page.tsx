...
export default function AccountsPayablePage() {
    ...
    const totalPayable = filteredPayments.reduce((sum, p) => sum + p.amount, 0);
    const postedPayments = filteredPayments.filter(p => p.isPosted);
    const unpostedPayments = filteredPayments.filter(p => !p.isPosted);
        ...
        <div className="max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-amber-50 rounded-lg p-4 border border-amber-200">
                    ...
                    <div className="text-sm text-amber-600">Unposted Payments</div>
                </div>
                <div className="bg-green-50 rounded-lg p-4 border border-green-200">
                    <div className="text-2xl font-bold text-green-900">
                        {postedPayments.length}
                    </div>
                    <div className="text-sm text-green-600">Posted Payments</div>
                </div>
            </div>
                            ...
                            filteredPayments.map(payment => (
                                <tr key={payment.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4">
                                        <span className={`px-2 py-1 rounded text-xs font-medium ${payment.isPosted ? "bg-green-100 text-green-700" :
                                            "bg-amber-100 text-amber-700"
                                            }`}>
                                            {payment.isPosted ? "Posted" : "Unposted"}
                                        </span>
                                    </td>
                                </tr>
                            ))
